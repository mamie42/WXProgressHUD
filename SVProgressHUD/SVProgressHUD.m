...
#pragma mark - Setters
+(void)setDefaultPosition:(SVProgressHUDPosition)positon{
    [[self sharedView] setDefaultPosition:positon];
}
+ (void)setStatus:(NSString*)status {
...
+(void)showMessageWithStatus:(nonnull NSString*)status withPosition:(SVProgressHUDPosition)positon{
      NSTimeInterval displayInterval = [self displayDurationForString:status];
    [[self sharedView]showMessageWithText:status withPosition:positon duration:displayInterval];
 
}
-(void)showMessageWithText:(NSString*)status withPosition:(SVProgressHUDPosition)positon duration:(NSTimeInterval)duration{
    _defaultPosition=positon;
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            /******/
            if(positon==SVProgressHUDPositionCenter){
              [strongSelf setOffsetFromCenter:UIOffsetZero];  
            }else if(positon==SVProgressHUDPositionBoutton){
                
                [strongSelf setOffsetFromCenter:UIOffsetMake(0,(strongSelf.center.y-100))];
            }
            [strongSelf setBackgroundColor:[UIColor lightGrayColor]];
            strongSelf.defaultStyle = SVProgressHUDStyleDark;
...
-(void)setDefaultPosition:(SVProgressHUDPosition)defaultPosition{
         if (!_isInitializing) _defaultPosition = defaultPosition;
}
- (void)setDefaultStyle:(SVProgressHUDStyle)style {
